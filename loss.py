...
class LossTask(DecodingTask):
    ...
    def _decoder_forward(
        self, audio_features: Tensor, tokens: Tensor, init_tokens_length: int
    ):
        ...
        loss = self._loss_from_logits(
            logits[:, (init_tokens_length - 1) :],
            tokens[:, init_tokens_length:],
            loss_fct,
        )
    ...
    def run(self, mel: Tensor, label: Union[str, torch.Tensor]) -> List[LossResult]:
        ...
        input_tokens: Tensor = (
            torch.tensor(label).repeat(n_audio, 1).to(audio_features.device)
        )  # 标签重复到 batch
        eos_tokens: Tensor = (
            torch.tensor([[tokenizer.eot]]).repeat(n_audio, 1).to(audio_features.device)
        )  # 终止 token
...
) -> Union[LossResult, List[LossResult]]:

    single = mel.ndim == 2
    if single:
        mel = mel.unsqueeze(0)

    # === 关键修复：Whisper encoder 要求 float32 ===
    mel = mel.float()
...
def get_loss_single_segment(
    ...
    **options,
    ...
    result: LossResult = get_loss_from_mel(
        model, mel, label, confidence, correct_first_word, DecodingOptions(**options)
    )
    loss = result.loss
    if loss.nelement() > 1:
